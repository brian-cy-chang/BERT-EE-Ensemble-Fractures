...
class BERT_EE(nn.Module):
    ...
    def __init__(self):
        ...
        #self.fine_tuned_path = Config.getlist("general", "fine_tuned_path")
        ...
        #self.__pretrained_model_name_or_path = Config.getlist("model", "pretrained_model_name_or_path")
    ...
    def __to_device(self):
        ...
        if torch.cuda.is_available():
            ...
            # self.__model = torch.nn.DataParallel(self.__model.module)
            self.cuda()
        else:
    ...
    def __get_pretrained(self, path):
        ...
        # self.__model = BERT_EE_model.from_pretrained(path, output_hidden_states=True)
    ...
    def __build(self, use_fine_tuned):
        ...
        if use_fine_tuned:
            ...
            #for pretrained in self.fine_tuned_path:
            #    self.__get_pretrained(pretrained)
        ...
        if not self.use_fine_tuned:
            ...
            for e in range(self.__epochs):
                ...
                for step, batch in enumerate(tqdm(self.__train_loader, desc="training")):
                    ...
                    sample_idx, filenames, bert_labels, bert_tokens, tokens, tokens_indices, labels, logits, total_loss, loss, last_hidden_states = \
                ...
                Logger.info(" training loss : {0:.3f}".format(mean_loss))
                ...
                if 'ner' in self.__best_result:
                    Logger.info('ner ', '{:.4f}'.format(self.__best_result['ner']['f1']))
                if 're' in self.__best_result:
                    Logger.info('re ', '{:.4f}'.format(self.__best_result['re']['f1']))
                if self.__best_result.keys().__len__() > 2:
                    Logger.info('avg ', '{:.4f}'.format(self.__best_result['overall_f1']))

                Logger.info('>>>>> current:')
                if 'ner' in results:
                    Logger.info('ner ', '{:.4f}'.format(results['ner']['f1']))
                if 're' in self.__best_result:
                    Logger.info('re ', '{:.4f}'.format(results['re']['f1']))
                if results.keys().__len__() > 2:
                    Logger.info('avg ', '{:.4f}'.format(
                        np.mean([results[t]['f1'] * self.__multi_task_training.get_weights(t) for t in self.__tasks])))
    ...
    def evaluate_model(self):
        ...
        with torch.no_grad():
            ...
            Logger.info(" evaluation loss : {0:.3f}".format(mean_loss))
    ...
    def __do_predict(self, task, dataset):
        ...
        with torch.no_grad():
            ...
            for step, batch in enumerate(self.__test_loader):
                ...
                for fname, bert_token, logit in zip(filenames, bert_tokens, logits):
                    basename = os.path.basename(fname).split("/")[0]
                    tmp = dict()
                    # tmp['sample_idx'] = sample_idx
                    tmp['filename'] = fname
                    # tmp['bert_labels'] = bert_labels
                    tmp['bert_tokens'] = bert_token
                    tmp['hidden_state'] = last_hidden_states.detach().cpu()
                    # tmp['logit'] = logit.detach().cpu()
                    # tmp['output'] = output
                    # tmp['hidden_state'] = hidden_state
                    # tmp['hiddenState'] = bert_tokens + logits
                    # tmp['labels'] = labels
                    # tmp['preds'] = preds

                    hiddenState.append(tmp)

                    with open(Config.get_resultfolder() + '/{}_logits.pkl'.format(basename), 'wb') as f:
                    #with open(Config.get_resultfolder() + '/{}_hiddenState.pkl'.format(basename), 'wb') as f:
                        pickle.dump(hiddenState, f)
    ...
    def __update_step(self, task_name, dataset, task_weights, batch, total_loss, predict=False):
        ...
        outputs, last_hidden_states = self.__model(input_ids=input_ids, token_type_ids=None,
                               attention_mask=attention_mask,
        ...
        # for f, fname in enumerate(filenames):
        #for fname, bert_token, output in zip(filenames, bert_tokens, outputs):
            #basename = os.path.basename(fname).split("/")[0]

            #tmp = dict()
            #tmp['filename'] = fname
            #tmp['bert_token'] = bert_token
            #tmp['hidden_state'] = last_hidden_states
            #hidden_state.append(tmp)

            #with open(Config.get_resultfolder() + '/{}_logits.pkl'.format(basename), 'wb') as f:
                #pickle.dump(hidden_state, f)

        return sample_idx, filenames, bert_labels, bert_tokens, tokens, tokens_indices, labels, logits, total_loss, loss, last_hidden_states
    ...
    def save(self):
        ...
        #model_to_save = self.__model.module
