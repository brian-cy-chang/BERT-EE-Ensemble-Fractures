...
class RE_encoder():
    ...
    def build(self, tokenizer,use_fine_tuned,  predict=False):
        ...
        #self.classifier = nn.Sequential(nn.Linear(self.__bert_config.hidden_size, self.__bert_config.hidden_size//2),
        #                                nn.ReLU(),
        #                                nn.Linear(self.__bert_config.hidden_size//2, len( self.unique_labels)),
        #                                nn.ReLU())
    ...
    def show_evaluation_result(self,acc_all, precsions_all, recalls_all, f1_all,
        ...
        Logger.info('(labels only)    accuracy: {:.3f}%; precision: {:.3f}%; recall: {:.3f}%; FB1: {:.3f}% '.format(
            acc_labels * 100,
            np.mean(precsions_labels) * 100,
            np.mean(recalls_labels) * 100,
            np.mean(f1_labels) * 100))

        Logger.info('(w/ no_relation) accuracy : {:.3f}%; precision: {:.3f}%; recall: {:.3f}%; FB1: {:.3f}% '.format(acc_all * 100,
        ...
        fmt = '\t{{:<{}s}}\tprecision:{{:>{},.3f}}%  recall: {{:>{},.3f}}%  FB1: {{:>{},.3f}}%'.format(max_label_len+2,7, 7,7)  # widths only
