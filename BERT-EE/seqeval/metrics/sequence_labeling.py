...
def precision_recall_fscore_support(y_true: List[List[str]],
    ...
    def extract_tp_actual_correct(y_true, y_pred, suffix, *args):
        ...
        # y_true_flatten = [item for sublist in y_true for item in sublist + ['O']] #list(itertools.chain.from_iterable(y_true))
        # y_pred_flatten =  [item for sublist in y_pred for item in sublist + ['O']]
        ...
        for type_name in target_names:
            if mode=='partial':
                entities_true_type = list(entities_true.get(type_name, set()))
                entities_true_type.sort(key=lambda tup: tup[0])
                entities_pred_type = list(entities_pred.get(type_name, set()))
                entities_pred_type.sort(key=lambda tup: tup[0])
                ...
                for true in entities_true_type: #tqdm(entities_true_type, desc=type_name):
                    # entities_pred_type_matches = [p for p in entities_pred_type if p not in matched_pred and p[0]<=true[1] and
                    #                             len(set(range(true[0], true[1] + 1)).intersection(
                    #                                 range(p[0], p[1] + 1))) > 0]
                    # if entities_pred_type_matches.__len__()>0:
                    #     matched_pred.append(entities_pred_type_matches[0])
                    #     all.append([true, entities_pred_type_matches[0]])
                    #     match += 1

                    entities_pred_type_check = [p for p in entities_pred_type if p not in matched_pred and p[1] >= true[0]]
                    for pred in entities_pred_type_check:
                        if pred[0]>true[1]:
                            break

                        elif len(set(range(true[0],true[1]+1)).intersection(range(pred[0],pred[1]+1)))>0 :
                            all.append([true, pred])
                            # matched_true.append(true)
                            matched_pred.append(pred)
                            match += 1
                            break

                tp_sum = np.append(tp_sum, match)
            else:
...
def classification_report(y_true, y_pred,
    ...
    for row in zip(target_names, p, r, f1, s):
        reporter_dict.write(*row)
        reporter_str.write(*row)

    reporter_dict.write_blank()
    reporter_str.write_blank()
    ...
    for average in average_options:
        ...
        reporter_dict.write('{} avg'.format(average), avg_p, avg_r, avg_f1, support)
        reporter_str.write('{} avg'.format(average), avg_p, avg_r, avg_f1, support)
    reporter_dict.write_blank()
    reporter_str.write_blank()

    return reporter_str.report(),reporter_dict.report()
